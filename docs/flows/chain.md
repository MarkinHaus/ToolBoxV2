...
- `ChainPrinter` — used for colored terminal output in `ChainBuilder` and `ChainCLI`
- `ChainMetadata` — dataclass for chain metadata used by `ChainStorage` and `ChainCLI`
...
- Referenced by `cmd_show` in `manifest_cli`
- Referenced by `_get_prompt_message` in [`minicli`](minicli.md)
- Referenced by `_get_prompt_text` in `_isaa_cli_lagicy`
- Referenced by `cmd_delete` in `ContainerManager/cli`
- Referenced by `cmd_delete` in `user_manager`
- Referenced by `cmd_status` in `db_cli_manager`
- Referenced by `cmd_status` in `llm_gateway_cli`
- Referenced by `cmd_status` in `manifest_cli`
- Referenced by `cmd_status` in `observability_helper`
- Referenced by `cmd_test_mic` in `vad_interactive`
