...
- `DataAccessLog` aus UserDataAPI
- `a_format_class` aus flow_agent
- `get_filename` aus gist_control
- `decrypt_bytes` aus tcm_p2p_cli
- `_generate_with_banana` aus image_generator
- `_save_discord_audio_as_wav` aus voice_mode
- `_save_temp_audio` aus Stt
- `visit_AsyncFunctionDef` aus live

## Verwendet von

- builder — `_load_config`
- [toolbox_integration](toolbox_integration.md) — `_load_config`, `check_access`
- minio_manager — `_load_config`
- db_cli_manager — `_load_config`
- file_handler — `__iter__`
- [icli](../flows/icli.md) — `cli_create_job`
- pyshell — `_is_complete`
