...
class TestVFSGrepLimits(unittest.TestCase):
    ...
    def test_grep_case_insensitive_default(self):
        ...
        self.assertEqual(len(results), 3,
            f"Case-insensitive grep via (?i) muss alle Varianten finden {results=}")
...
class TestSearchVFS(unittest.TestCase):
    ...
    def test_search_filename_mode(self):
        ...
        self.assertEqual(len(results), 1, f"{results}")
...
class TestContextBuilding(unittest.TestCase):
    ...
    def test_modified_file_marked_in_context(self):
        ...
        try:
            ...
            res = self.vfs.write(vfs_path, "modified")
            self.assertTrue(res["success"], f"{res=}")
            ...
            self.assertFalse(vfs_file.is_dirty,
                f"Modifizierte Shadow-Datei muss is_dirty=False 'auto sync' haben {res}")
