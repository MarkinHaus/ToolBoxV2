...
TaskChainMode = LLMMode(
    name='TaskChainMode',
    description="Creating task chains in JSON format based on user requirements",
    system_msg="Analyze the provided requirements and create a task chain in valid JSON format. "
               "Break down the requirements into small, meaningful sub-tasks. "
               "Each task should have 'use', 'name', 'args', and 'return_key' fields. "
               "Ensure the JSON is properly formatted and follows the given schema.",
    post_msg='Assistant:',
    examples=[
        "Create a task chain for a simple calculator that can add and multiply two numbers.",
        '''{
  ...
  "dis": null
        ...
        "Create a task chain for a weather forecast system that fetches data and generates a report.",
        '''{
  ...
  "tasks": [
    ...
    },
    {
      "use": "agent",
      "name": "generateReport",
      "args": "location: $location analysis: $analysis",
      "return_key": "report"
    }
  ],
  "dis": null
...
class Task:
    def __init__(self, use: str = 'agent', name: str = 'self', args: str = '$user-input',
                 return_key: str = "return", **kwargs):
        self.use = TaskType(use)
        self.name = name
        self.args = args
...
class VariableSyntaxSystem:
    ...
    def parse_declarations(self, raw_string: str) -> List[tuple[str, Any]]:
        ...
        declarations = []
        for name, var_type, value in matches:
            typed_value = self._convert_type(value.strip(), var_type)
            self.declare_variable(name, typed_value)
            declarations.append((name, typed_value))
        return declarations
    ...
    def inject_variables(self, text: str) -> str:
        def replace_var(match):
            ...
            elif var_name == 'userinput'.upper():
                return f"{self.user_input}"  # Placeholder for user input
            else:
                return f"[UNDEFINED_VARIABLE:{var_name}]"
...
class ChainTreeExecutor:
    ...
    def execute(self, user_input: str, tasks, sum_up=False):
        ...
        for task in tasks:
            ...
            #    task['kwargs'] = {}
            task = Task(**task)
            # Inject variables and previous task results
            ...
            # Execute the task (placeholder for actual execution)
            result = self._execute_task(task.use, task.name, injected_args, **task.kwargs)
        ...
        if sum_up and len(self.task_results) > 3:
            return self._execute_task(TaskType.AGENT, name="isaa",
    ...
    def _inject_task_results(self, text: str) -> str:
        def replace_task_result(match):
            key = match.group(1)
            if key in self.task_results:
                return str(self.task_results[key])
            else:
    ...
    def _execute_task(self, task_type: TaskType, name: str, args: str, **t_task) -> Any:
        try:
            if task_type == TaskType.TOOL:
                return self.function_runner(name, **t_task)
            elif task_type == TaskType.AGENT:
                return self.agent_runner(name, args, **t_task)
            elif task_type == TaskType.CHAIN:
                return self.execute(args, **t_task)
            return f"Result of {task_type.value} task '{name}' with args: {args}"
