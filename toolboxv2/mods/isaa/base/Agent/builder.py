...
class AgentConfig(BaseModel):
    ...
    system_message: str = (
        """You are a production-ready autonomous agent with advanced capabilities."""
    )
...
class FlowAgentBuilder:
    ...
    def with_models(
        self, fast_model: str, complex_model: str = None
    ) -> "FlowAgentBuilder":
        """Set LLM models"""
    ...
    def add_api_key(self, provider: str, key: str) -> "FlowAgentBuilder":
        """
    ...
    def load_rate_limiter_config(self, config_path: str) -> "FlowAgentBuilder":
        ...
        self.config.rate_limiter.enable_rate_limiting = features.get(
            "rate_limiting", True
        )
        self.config.rate_limiter.enable_model_fallback = features.get(
            "model_fallback", True
        )
        self.config.rate_limiter.enable_key_rotation = features.get("key_rotation", True)
        self.config.rate_limiter.key_rotation_mode = features.get(
            "key_rotation_mode", "balance"
        )
    ...
    def enable_mcp_server(
        self, host: str = "0.0.0.0", port: int = 8000, server_name: str = None
    ) -> "FlowAgentBuilder":
        """Enable MCP server"""
    ...
    def load_mcp_tools_from_config(self, config_path: str | dict) -> "FlowAgentBuilder":
        ...
        self.config.mcp.config_path = (
            str(config_path) if isinstance(config_path, str) else None
        )
    ...
    def add_tools_from_module(
        self, module, prefix: str = "", category: str = None, exclude: list[str] = None
    ) -> "FlowAgentBuilder":
    ...
    # Preset personas
    def with_developer_persona(
        self, name: str = "Senior Developer"
    ) -> "FlowAgentBuilder":
        """Add and set pre-built developer persona"""
        return self.add_persona_profile(
            "developer",
            name=name,
            style="technical",
            tone="professional",
    ...
    def with_creative_persona(
        self, name: str = "Creative Assistant"
    ) -> "FlowAgentBuilder":
    ...
    def with_executive_persona(
        self, name: str = "Executive Assistant"
    ) -> "FlowAgentBuilder":
    ...
    def validate_config(self) -> dict[str, list[str]]:
        ...
        if (
            ...
            and self.config.active_persona not in self.config.persona_profiles
        ):
            issues["errors"].append(
                f"Active persona '{self.config.active_persona}' not found in profiles"
            )
    ...
    async def build(self) -> FlowAgent:
        ...
        with Spinner(message=f"Building Agent {self.config.name}", symbols="c"):
            ...
            try:
                ...
                if (
                    self.config.active_persona
                    and self.config.active_persona in self.config.persona_profiles
                ):
                    persona_data = self.config.persona_profiles[
                        self.config.active_persona
                    ].copy()
                    ...
                    if "format_config" in persona_data:
                        ...
                        format_config = FormatConfig(
                            response_format=ResponseFormat(
                                fc_data.get("response_format", "free-text")
                            ),
                            text_length=TextLength(
                                fc_data.get("text_length", "chat-conversation")
                            ),
                        )
                ...
                iprint(
                    f"   Checkpoints: {'✓' if self.config.checkpoint.enabled else '✗'}"
                )
                iprint(
                    f"   Persona: {active_persona.name if active_persona else 'Default'}"
                )
    ...
    async def _process_mcp_config(self, agent: FlowAgent):
        ...
        for server_name, server_config in mcp_config["mcpServers"].items():
            try:
                ...
                # Get session
                session = await self._mcp_session_manager.get_session(
                ...
                capabilities = await self._mcp_session_manager.extract_capabilities(
                    session, server_name
                )
                ...
                for tool_name, tool_info in capabilities.get("tools", {}).items():
                    ...
                    categories = [f"mcp_{server_name}", "mcp", server_name]
                ...
                if session_obj.rule_set:
                    tool_names = [
                        f"{server_name}_{t}" for t in capabilities.get("tools", {}).keys()
                    ]
    ...
    def _create_mcp_tool_wrapper(
        self, server_name: str, tool_name: str, tool_info: dict, session
    ):
        ...
        async def tool_wrapper(**kwargs):
            try:
                ...
                arguments = {
                    ...
                    if v is not None or k in required_params
                }
    ...
    def create_developer_agent(
        cls, name: str = "DeveloperAgent", with_mcp: bool = True, with_a2a: bool = False
    ) -> "FlowAgentBuilder":
    ...
    def create_general_assistant(
        cls, name: str = "AssistantAgent", full_integration: bool = True
    ) -> "FlowAgentBuilder":
    ...
    def create_executive_agent(
        cls, name: str = "ExecutiveAgent", with_integrations: bool = True
    ) -> "FlowAgentBuilder":
...
async def example_usage():
    ...
    agent2 = await (
        ...
        .with_rate_limiter(enable_model_fallback=True, key_rotation_mode="drain")
        ...
        .set_model_limits(
            "vertex_ai/gemini-2.5-pro", rpm=2, input_tpm=32000, is_free_tier=True
        )
        .build()
    )
