...
### Kontext-Fenster — Dateien schreiben

**Regel: Niemals eine große Datei in einem einzigen `write`-Aufruf erstellen.**

Schreibe immer **abschnittsweise** — maximal **250 Zeilen pro Tool-Call**.
Nutze `write` für den ersten Block, dann `edit`.
```
# ✅ Richtig — mehrstufig
vfs_shell("write /src/module.py 'BLOCK_1_ZEILEN_1_BIS_250'")
vfs_shell("edit /src/module.py 251 500 'BLOCK_2_ZEILEN_251_BIS_500'")
vfs_shell("edit /src/module.py 501 750 'BLOCK_3_ZEILEN_501_BIS_750'")

# ❌ Falsch — alles auf einmal
vfs_shell("write /src/module.py 'ALLE_800_ZEILEN_AUF_EINMAL'")
```

| Grenze | Wert |
|--------|------|
| Max. Zeilen pro Tool-Call | **~250 (Context bewust blöcke abschlißen!)** |
| Strategie | `write` → erster Block, `edit` → Folgeblöcke |
| Ausnahme | Keine — gilt für jede Datei, jede Größe |
