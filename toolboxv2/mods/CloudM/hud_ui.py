...
anpassung in meiner tauri app. sie ligt in C:\Users\Markin\Workspace\ToolBoxV2\toolboxv2\simple-core unzwar soll die destop bar aus toolboxv2\tbjs\src\ui\components\Desktop unzwar soll der schorkut nicht mher das quik qaptur öffnen sonder von der app den hud mode öffenen suche zuerst nach informationen im code un wo die anpassung prezise umgestezt werden muss. im tray icon gibt es berits eine fuction um in den hud modus zu wächsel diese muss verwendet werden.
"""
...
from .mini import get_service_status, PID_DIRECTORY
...
class CloudMHud(HudWidget):
    ...
    def __init__(self):
        super().__init__(
            widget_id="cloudm_overview",
            title="CloudM Overview",
            icon="☁️"
        )
        # Register standard actions
        self.action("refresh")(self.on_refresh)
        self.action("restart_service")(self.on_restart_service)
        self.action("sync_user")(self.on_sync_user)
        self.action("toggle_theme")(self.on_toggle_theme)
    ...
    def _render_login_prompt(self) -> str:
        return self.card(
            ...
            actions=self.button("Login", "navigate", {"path": "/web/assets/login.html"}, style="primary", icon="login")
        )

    def _render_profile_card(self, username, level, uid) -> str:
        # User Level Badge Color
        level_color = "#22c55e" if level >= 1 else "#fbbf24";
        if level == -1: level_color = "#f43f5e"  # Admin red
    ...
    def _render_system_status(self) -> str:
        ...
        return self.card(
            ...
            actions=self.button("Full Status", "navigate", {"path": "/api/CloudM.UserDashboard/main"},
                                style="secondary", icon="bar_chart")
        )
    ...
    async def on_sync_user(self, app, payload, conn_id, request):
        ...
        if user_id:
            ...
            await self.push_notification(app, conn_id, "Profile synchronized", level="success")
            # Refresh UI
            html = await self.render(app, request)
            await self.push_update(app, conn_id, html)
        return {"status": "ok"}
    ...
    async def on_restart_service(self, app, payload, conn_id, request):
        ...
        if service:
            await self.push_notification(app, conn_id, f"Restarting {service}...", level="warning")
            # Actual restart logic would go here via mini or App
            await self.push_notification(app, conn_id, f"{service} restarted", level="success")
        return {"status": "ok"}
...
@export(mod_name=Name, api=True, version=version)
async def hud_action(app: App, action: str, payload: dict, conn_id: str, request: RequestData = None):
    """
