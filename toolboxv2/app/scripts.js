...
let pointLightSto = []
let ambientLightSto = new THREE.AmbientLight(0x181823);
...
// Create a point light
    const pointLight = new THREE.PointLight(0x181823, 1, 100);
...
// Create an ambient light darg 0x181823
    const ambientLight = new THREE.AmbientLight(0x181823);
...
function loadDarkModeState() {
    ...
    scene.remove(ambientLightSto)
    ambientLightSto = new THREE.AmbientLight(color);
    scene.add(ambientLightSto)
    let position = [0,0,0]
 ...
 * @returns {THREE.Group} - A group containing the Sierpinski Triangle geometry
 */
function createSierpinskiTriangle(depth, size) {
    ...
    function createSierpinski(depth, size, group) {
        if (depth === 0) {
            const triangle = new THREE.TetrahedronGeometry(size*0.6);
            const mash = new THREE.Mesh(triangle, milkGlassMaterial)
            mashs.push(mash)
            group.add(mash);
        } else {
            ...
            const bottomLeftGroup = new THREE.Group();
            ...
            const bottomRightGroup = new THREE.Group();
            ...
            const bottomBackGroup = new THREE.Group();
...
window.onwheel = e => {
    ...
    //     sk2 = sk;
    // } Shöner aus der färe fall scrollen
