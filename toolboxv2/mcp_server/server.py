...
class ToolBoxV2MCPServer:
    ...
    def __init__(self, config: Optional[MCPConfig] = None):
        ...
        self.sessions = FlowSessionManager(
            max_sessions=self.config.max_sessions, timeout=self.config.session_timeout
        )
        ...
        if self.config.enable_flows:
            tools.extend(
                [
                    {
                        "name": "flow_start",
                        "description": "Start a workflow with session management.",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "flow_name": {
                                    "type": "string",
                                    "description": "Flow name (use toolbox_info to discover)",
                                },
                                "session_id": {
                                    "type": "string",
                                ...
                                "kwargs": {
                                    "type": "object",
                                    "description": "Flow initialization parameters",
                                    "default": {},
                                },
                            },
                            "required": ["flow_name"],
                    ...
                    {
                        "name": "flow_continue",
                        "description": "Continue an active flow session with input.",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "session_id": {
                                    "type": "string",
                                    "description": "Session ID from flow_start",
                                },
                                "input_data": {
                                    "type": "object",
                    ...
                    {
                        "name": "flow_status",
                        "description": "Get status of a flow session.",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "session_id": {
                    ...
                    {
                        "name": "flow_list",
                        "description": "List all active flow sessions.",
            ...
            return json.dumps(
                {
                    ...
                    "modules": list(getattr(self._app, "functions", {}).keys())
                    if self._app
                    else [],
                    "flows": list(getattr(self._app, "flows", {}).keys())
                    if self._app
                    else [],
                    "docs_available": hasattr(self._app, "docs_reader")
                    if self._app
                    else False,
                },
                indent=2,
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> List[ToolResult]:
        ...
        try:
            ...
            if name == "toolbox_execute":
                ...
                result = await self._handle_flow_continue(arguments)

            elif name == "flow_status":
                result = await self._handle_flow_status(arguments)

            elif name == "flow_list":
                result = await self._handle_flow_list()
    ...
    async def _handle_flow_start(self, arguments: Dict) -> ToolResult:
        """Handle flow_start tool."""
        start_time = time.time()
        flow_name = arguments.get("flow_name")

        if not self._app or not hasattr(self._app, "flows"):
            return ToolResult(
                success=False,
        ...
        if flow_name not in self._app.flows:
            ...
            return ToolResult(
                success=False,
                content=f"❌ Flow '{flow_name}' not found.\n\n**Available flows:**\n"
        ...
        session = await self.sessions.create(
            flow_name=flow_name, session_id=arguments.get("session_id")
        )
        ...
        return ToolResult(
            ...
            + f"- **Status**: Ready\n\n"
            + f"Use `flow_continue(session_id='{session.session_id}', input_data={{...}})` to proceed.",
            execution_time=time.time() - start_time,
        )

    async def _handle_flow_continue(self, arguments: Dict) -> ToolResult:
        """Handle flow_continue tool."""
        start_time = time.time()
        ...
        await self.sessions.update(
            ...
            history_entry=f"Input: {json.dumps(input_data)}",
        )

        # Execute flow step
        try:
            result_content = (
                f"✅ Flow '{session.flow_name}' processed input.\n\n"
            ...
            await self.sessions.update(session_id, state=FlowState.ERROR)
            return ToolResult(
                success=False,
                content=f"❌ Flow error: {e}",
                execution_time=time.time() - start_time,
                error=str(e),
            )

    async def _handle_flow_status(self, arguments: Dict) -> ToolResult:
        """Handle flow_status tool."""
        start_time = time.time()
        session_id = arguments.get("session_id")
        ...
        return ToolResult(
            ...
            + f"- **History**: {len(session.history)} entries",
            execution_time=time.time() - start_time,
        )

    async def _handle_flow_list(self) -> ToolResult:
        """Handle flow_list tool."""
        start_time = time.time()
        ...
        if not sessions:
            ...
            for s in sessions:
                content += f"- `{s['session_id']}`: {s['flow_name']} ({s['state']})\n"
    ...
    def _setup_handlers(self):
        ...
        async def handle_call_tool(
            ...
            return [types.TextContent(type="text", text=r.content) for r in results]
