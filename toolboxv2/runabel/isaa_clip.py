...
def set_up(app, speek_mode, trigger_word):
    ...
    return self_agent_config, print_, spek, get_input, speek_mode
...
def run_clipy_isaa(app, speak_mode=False):
    # Trigger word to process the text
    trigger_word = 'isaa-run'

    print(f"Script running in the background")

    self_agent_config, print_, spek, get_input, speak_mode = set_up(app, speak_mode, trigger_word)
    buffer = ' ' * 8
    alive = True
    while alive:
        ...
        if event.event_type == keyboard.KEY_DOWN:
            ...
            print(buffer[len(buffer)-8:], buffer.endswith("isaa-exit"), buffer.endswith(trigger_word), end='\r')
            ...
            if buffer.endswith(trigger_word):
                ...
                res = run_agent_clip(app, user_text, self_agent_config, print_, spek, get_input, speak_mode)
