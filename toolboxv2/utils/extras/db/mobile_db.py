...
class MobileDB:
    ...
    def put(self, path: str, data: bytes,
        ...
        with self._lock:
            ...
            try:
                ...
                row = conn.execute(
                    "SELECT version FROM blobs WHERE path = ?", (path,)
                ).fetchone()
            ...
            metadata = BlobMetadata(
                path=path, size=len(data), checksum=checksum,
                local_updated_at=now, sync_status=sync_status,
                version=version, content_type=content_type, encrypted=encrypted,
            )
    ...
    def start_watch_poller(self, interval: float = 0.5):
        """Start background thread polling sync_log for cross-process changes."""
        if getattr(self, "_poller_thread", None) is not None:
            return
        self._poller_stop = threading.Event()
        self._poller_last_id = self._get_max_log_id()
        self._poller_thread = threading.Thread(
            target=self._poll_loop, args=(interval,), daemon=True
        )
        self._poller_thread.start()

    def stop_watch_poller(self):
    ...
    def _get_max_log_id(self) -> int:
        conn = self._get_connection()
        row = conn.execute("SELECT COALESCE(MAX(id), 0) AS m FROM sync_log").fetchone()
        return row["m"]
    ...
    def _poll_loop(self, interval: float):
        while not self._poller_stop.wait(interval):
            try:
                conn = self._get_connection()
                rows = conn.execute(
                    "SELECT id, path, action FROM sync_log WHERE id > ? ORDER BY id",
                    (self._poller_last_id,)
                ).fetchall()
                for r in rows:
                    self._poller_last_id = r["id"]
                    meta = self.get_metadata(r["path"])
                    self._notify_watchers(r["path"], r["action"], meta)
            except Exception as e:
                get_logger().error(f"Watch poller error: {e}")

    def _notify_watchers(self, path: str, action: str, data: Any):
        key = (path, action, getattr(data, "version", None) if data else None)
        if not hasattr(self, "_recent_notify"):
            self._recent_notify = {}
        now = time.time()
        # purge old
        self._recent_notify = {k: t for k, t in self._recent_notify.items() if now - t < 5}
        if key in self._recent_notify:
            return
        self._recent_notify[key] = now
    ...
    def _check_size_limit(self):
        ...
        if current_size <= self.max_size_bytes:
            return
        # Advisory lock via sidecar file — only one process cleans at a time
        lock_path = self.db_path.with_suffix(self.db_path.suffix + ".cleanup.lock")
        try:
            import fcntl
            with open(lock_path, "w") as lf:
                ...
                get_logger().warning(
                    f"Database size ({current_size / 1024 / 1024:.1f}MB) exceeds limit. of ({self.max_size_bytes / 1024 / 1024:.1f}MB).  Cleaning..."
                )
                self.cleanup_old_synced(target_size=int(self.max_size_bytes * 0.8))
        except ImportError:
            # Windows fallback — just run, accept duplicate work
            self.cleanup_old_synced(target_size=int(self.max_size_bytes * 0.8))
