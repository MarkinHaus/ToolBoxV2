"""
Local conftest for test_utils - ensures test_toolbox.py always uses a real App.

This overrides the global reset_app_state_before_test fixture to NOT reset
the App singleton, allowing test_toolbox.py to maintain its real App instance
across all its tests.
"""
...
# ============================================================================
# Singleton and App State Cleanup
# ============================================================================
...
try:
    ...
    # Fallback if import fails - mirrors a_util.reset_app_singleton
    def reset_app_singleton(force_exit: bool = False):
        """Fallback reset function for full test isolation."""
        ...
        # 2. Reset ALL Singleton instances
        try:
...
# Test files that need a persistent App across all their tests
# These files use setUpClass to create one App for all tests in the class
PERSISTENT_APP_TEST_FILES = {
    "test_toolbox.py",      # toolboxv2/tests/test_utils/
    "test_toolboxv2.py",    # toolboxv2/tests/test_mods/ - loads all mods
}
...
def reset_app_state_before_test(request):
    """
    Automatically reset App singleton state before and after each test.
    This ensures every test is fully independent with a clean App state.

    Exception: Files in PERSISTENT_APP_TEST_FILES manage their own App lifecycle
    via setUpClass/tearDownClass and should not be reset between tests.
    """
    # Get test file name
    test_file = ""
    ...
    if test_file in PERSISTENT_APP_TEST_FILES:
        # Don't reset - these tests manage their own App lifecycle
        yield
        return

    # Force reset before test - every test starts fresh
    reset_app_singleton(force_exit=True)
    yield
    # Force reset after test - clean up for next test
    reset_app_singleton(force_exit=True)
...
# Marker to identify tests that need a real App
def pytest_configure(config):
    config.addinivalue_line(
        "markers", "real_app: mark test as requiring a real App instance"
    )
...
def reset_app_state_before_test(request):
    """
    Override the global fixture for test_toolbox.py.

    For test_toolbox.py: Do NOT reset - let it manage its own real App.
    For other tests: Use the default behavior (reset before/after).
    """
    # Check if this test is from test_toolbox.py
    test_file = request.fspath.basename if hasattr(request, 'fspath') else ""

    if test_file == "test_toolbox.py":
        # Don't reset for test_toolbox.py - it needs a persistent real App
        yield
        return

    # For all other tests, use the standard reset behavior
    from toolboxv2.tests.a_util import reset_app_singleton
    reset_app_singleton(force_exit=True)
    yield
    reset_app_singleton(force_exit=True)
